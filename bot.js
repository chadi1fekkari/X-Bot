...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = [`${client.guilds.size} Server`,'X.help','Type X.help',`${client.users.size} Members`,'X.inv','By: !  SizGooo ツ'];
    var i = -1;
    var j = 0;
...
if (message.content.startsWith('X.help')) { /// This is The DMS Code Send The Help In DMS // Code By NotGucci
    let pages = [`
***__وصف عن البوت__***
**
...
client.on('message', message => {
    if(!message.guild) return;
    if (message.author.bot) return;
      let id = message.author.id,prefix="X.";//البريفكس
...
client.on('messageUpdate', (oldRebel, newRebel) => {
    console.log("عضو مآ يحآول التعديل.");
...
${prefix}queue ? لمعرفة قآئمة التشغيل
 ...
 client.on('message', message => {
            if(!message.channel.guild) return;
let args = message.content.split(' ').slice(1).join(' ');
if (message.content.startsWith('X.users')){
 if(!message.author.id === '458640422420480012') return;
...
client.on('message' , async (message) => {
    if(message.content.startsWith("X.topinvite")) {
if(message.author.bot) return;
